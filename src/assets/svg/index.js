import X from "./X.svg?react";
import FileUpload from "./FileUpload.svg?react";
import SearchMore from "./searchMore.svg";
import Chatto from "./chatto.svg?react";
import ArrowDown from "./ArrowDown.svg?react";
import Person from "./Person.svg?react";
import ChemiIconFull from "./ChemiIconFull.svg?react";
import EyeOpened from "./eyeOpened.svg";
import EyeClosed from "./eyeClosed.svg";
import NavChemi from "./NavChemi.svg?react";
import NavLover from "./NavLover.svg?react";
import NavMBTI from "./NavMBTI.svg?react";
import NavMore from "./NavMore.svg?react";
import NavSome from "./NavSome.svg?react";
import NavStat from "./NavStat.svg?react";

export { X, FileUpload, SearchMore, Chatto, ArrowDown, Person, ChemiIconFull, EyeClosed, EyeOpened, NavChemi, NavLover, NavMBTI, NavMore, NavSome, NavStat };

