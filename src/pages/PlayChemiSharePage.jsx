...
import { getChemiGuestDetail, getChemiQuiz } from "@/apis/api"; // 실제 API 호출 함수
import {
  Header,
  ChatList,
  FileUpload,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rid = detail?.result?.result_id ?? null;
        ...
        if (rid != null) {
          try {
            await getChemiQuiz(rid); // 200이면 존재
            ...
            if (e?.response?.status === 404) {
              ...
              // 기타 에러는 일단 비활성화 처리하고 콘솔만
              console.warn("quiz check failed:", e);
              setQuizAvailable(false);
            }
          }
        }
        ...
        <div className="w-[214px] mt-52.5 flex flex-col items-center justify-start gap-1.5">
          <div className="w-full flex justify-between items-center">
            ...
            <div className="relative group">
              <button
                onClick={() =>
                  quizResultId && navigate(`/play/quiz/chem/${quizResultId}`)
                }
