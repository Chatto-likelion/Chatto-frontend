...
export default function PlayMyPage() {
  ...
  const ensureUuid = async (type, resultId) => {
    if (!resultId) return null;

    const typeSlug = type === 3 ? "mbti" : type === 2 ? "some" : "chem";

    let uuid = null;
    try {
      const got = await getUUID(typeSlug, resultId);
    ...
    if (!uuid) {
      const created = await postUUID(typeSlug, resultId);
  ...
  const handleQuiz = async (type, resultId) => {
    setQuizLoading(true);
    try {
      await postQuiz10(type, resultId);
      const uuid = await ensureUuid(type, resultId);
  ...
  const handleGoQuiz = async (type, resultId) => {
    try {
      const uuid = await ensureUuid(type, resultId);
      navigate(`/play/quiz/${resultId}/${encodeURIComponent(uuid)}`);
    } catch (err) {
                    ...
                    >
                      <div className="w-full flex flex-col justify-start items-center">
                        ...
                        {(() => {
                          ...
                          const people =
                            chatInfo?.people_num ?? item.people_num;

                          const isSelected =
                            selectedChatId != null &&
                            item.chat === selectedChatId;
                          const base =
                      ...
                      <div className="w-full flex justify-start items-center gap-4 ">
                        ...
                        {!loading && !item.is_quized ? (
                          <button
                            onClick={() =>
                              handleQuiz(item.type, item.result_id)
                            }
                            disabled={quizLoading}
                            className="w-17.5 h-6.5 px-1.5 py-0.75 border border-secondary text-secondary text-button rounded hover:bg-primary-light hover:text-primary-dark cursor-pointer"
                          >
                            퀴즈 생성
                          </button>
                        ) : (
                          <button
                            onClick={() =>
                              handleGoQuiz(item.type, item.result_id)
                            }
                            disabled={quizLoading}
                            className="w-17.5 h-6.5 px-1.5 py-0.75 border border-secondary text-secondary text-button rounded hover:bg-primary-light hover:text-primary-dark cursor-pointer"
                          >
